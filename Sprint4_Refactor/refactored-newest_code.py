...
class AutonomousVideoStream:
    def __init__(self, drone, cap, cap_lbl, face_recognizer,
                 known_face_encodings, known_face_names, face_detection_var, joystick_controller):
    ...
    def video_stream(self):
        ...
        if frame is not None:
            ...
            if selected_name == "Disable":
                ...
                self._autonomous_logic(frame, cur_gray_frame, w, h)
    ...
    def _autonomous_logic(self, frame, cur_gray_frame, w, h):
        selected_name = self.face_detection_var.get()

        # --- Thread: execute movement commands ---
        def read_movement():
            if self.movement:
...
class DroneController:
    def __init__(self):
        ...
        self.video_stream = AutonomousVideoStream(
            drone=self.drone,
            cap=self.cap,
